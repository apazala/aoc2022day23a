...
internal class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        int imin = 1000000000, imax = -1000000000, jmin = 1000000000, jmax = -1000000000;
        foreach(Elf elf in elfList){
            if(elf.currentPosition.i < imin)
                imin = elf.currentPosition.i;
            if(elf.currentPosition.i > imax)
                imax = elf.currentPosition.i;
            if(elf.currentPosition.j < jmin)
                jmin = elf.currentPosition.j;
            if(elf.currentPosition.j > jmax)
                jmax = elf.currentPosition.j;
        }

        int result = (imax-imin+1)*(jmax-jmin+1);
        result-= elfList.Count;
